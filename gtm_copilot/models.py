...
# ---------------------------------------------------------------------------
# Phase 6 / v2 Models: Outreach Generation
# ---------------------------------------------------------------------------


class OutreachInput(BaseModel):
    """Input payload for generating personalized sales outreach sequences."""

    account_brief: AccountBrief = Field(
        ...,
        description="Synthesized AccountBrief containing company overview, pain points, and talk tracks.",
    )
    contact_name: Optional[str] = Field(
        default=None,
        description="Name of the specific prospect or buyer contact (if known).",
    )
    contact_role: Optional[str] = Field(
        default=None,
        description="Title or role of the contact (e.g., VP of Sales Operations).",
    )
    contact_linkedin_or_notes: Optional[str] = Field(
        default=None,
        description="Optional unstructured notes, recent LinkedIn posts, or context about the contact.",
    )


class EmailVariant(BaseModel):
    """A personalized cold outreach email variant with a distinct strategic tone."""
    ...
    tone_label: str = Field(
        ...,
        description="Strategic tone label (e.g. 'direct', 'consultative', 'curiosity-driven').",
    )


class FollowUpVariant(BaseModel):
    """A follow-up touchpoint in a multi-step outbound sales cadence."""

    subject: str = Field(..., description="Follow-up email subject line.")
    body: str = Field(..., description="Follow-up email body copy.")
    send_after_days: int = Field(
        ...,
        description="Suggested waiting period in days before sending this follow-up touch.",
    )
    sequence_position: int = Field(
        ...,
        description="Order index in the follow-up sequence (e.g. 1 for Touch 2, 2 for Touch 3).",
    )


class OutreachOutput(BaseModel):
    """Structured personalized outreach package generated by OutreachAgent."""

    contact_name: Optional[str] = Field(
        default=None,
        description="Contact name addressed, or None if company-level generic.",
    )
    email_variants: List[EmailVariant] = Field(
        ...,
        min_length=1,
        description="List of 3-5 initial cold email variants across distinct tones.",
    )
    follow_up_sequence: List[FollowUpVariant] = Field(
        ...,
        min_length=1,
        description="List of 2-3 progressive follow-up emails.",
    )
    personalization_notes: List[str] = Field(
        default_factory=list,
        description="List of specific signals, triggers, or pain points used to tailor the outreach.",
    )
    source_grounding: List[str] = Field(
        default_factory=list,
        description="Traceability snippets from the AccountBrief and playbooks used as grounding.",
    )


class FactCheckedOutreach(BaseModel):
    """Final verified Outreach package with per-claim audit results and faithfulness score."""

    outreach: OutreachOutput = Field(..., description="The synthesized outreach package.")
    fact_checks: List[FactCheckResult] = Field(
        default_factory=list,
        description="Detailed verification results for all audited statements in the outreach.",
    )
    overall_faithfulness_score: float = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
        description="Proportion of claims supported by source evidence (0.0 to 1.0).",
    )
    flagged_claims: List[str] = Field(
        default_factory=list,
        description="List of claims in the outreach that failed fact-checking (unsupported/hallucinated).",
    )
